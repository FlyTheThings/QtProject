...
#include "ui_analogwidget.h"

analogWidget::analogWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::analogWidget)
{
    ui->setupUi(this);
...
void analogWidget::initButtonGroup(void)
{
...
void analogWidget::echoMessagePacket(datalink_message_t *msg)
{
    quint8 echoMsg;
    float  tmpF;
    quint8 index;
...
void analogWidget::slot_cmdButtonClicked(int ButtonID)
{
    quint8   Data[12];
    float    tmpF;
    quint8   index=0;
...
void analogWidget::slot_ADTimerDisplay()
{
   quint8 Data[2];
...
void analogWidget::showEvent(QShowEvent * event)
{
    Q_UNUSED(event);
    p_Timer->start(500);
}

void analogWidget::hideEvent(QHideEvent * event)
{
    Q_UNUSED(event);

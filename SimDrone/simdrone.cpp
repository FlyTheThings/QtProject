    ...
    ui(new Ui::simDrone)
{
    ...
    connect(ui->send_attitude, SIGNAL(clicked()), this, SLOT(send_attitude_msg()));

    // disable send_heartbeat button
    ui->send_heartbeat->setEnabled(false);
    ui->send_attitude->setEnabled(false);
...
void simDrone::updateClientDetail() {
    ...
    ui->send_attitude->setEnabled(true);

}

/*
 * send the attitude message to the app
 */
void simDrone::send_attitude_msg() {
    quint16 checksum;
    quint8 payload[50];
    quint8 dataToSend[64];
    quint8 length = 12, index;

    /*
    * get the uav flight attitude message.
    */
    getAttitudeMessage();

    frame_push_float(payload, 0, flightAttiData.roll);
    frame_push_float(payload, 4, flightAttiData.yaw);
    frame_push_float(payload, 8, flightAttiData.pitch);
    ...
    dataToSend[6] = UAV_MSG_FCC_ATTITUDE;
...
void simDrone::getAttitudeMessage() {
    flightAttiData.roll = ui->row_angle->text().toFloat();
    flightAttiData.yaw = ui->yaw_angle->text().toFloat();
    flightAttiData.pitch = ui->pitch_angle->text().toFloat();
}
